<script lang="ts">
    ...
    let data: Writable<{[key in string]: Array<DataEntry>}> = !localStorage ? writable({}) : writable(JSON.parse(localStorage.getItem("data") || "{}"));
    ...
    async function upload_file(e: Event) {
        ...
        if(target.files?.length == 1) {
            ...
            for(let i = 0;i < entries_cnt;i++) {
                ...
                let entry: DataEntry = {
                    distance1, distance2, distance3, latitude, longitude, pulse
                };
                data.update((d) => {
                    let k = file.name;
                    return {...d, [k]: d[k].concat([entry])};
                });
                //data[file.name].push(entry);
            }
        }
    }
